<<<<<<< HEAD
CREATE OR REPLACE FORCE VIEW "CORNSALAD"."V_PROJECTLIST" ("PROJECT_ID", "IMAGE_RENAME", "PROJECT_NAME", "PROJECT_CATEGORY_ID", "PROJECT_CATEGORY_NAME", "CATEGORY_SUB_ID", "CATEGORY_SUB_NAME", "CATEGORY_ID", "CATEGORY_NAME", "START_DATE", "END_DATE", "MEMBER_ID", "MEMBER_NAME", "TARGET_AMOUNT", "DDAY", "TOTAL_AMOUNT", "PERCENT","REPORT_COUNT") AS 
  (SELECT P.PROJECT_ID AS PROJECT_ID, IMAGE_RENAME, PROJECT_NAME, P.PROJECT_CATEGORY_ID AS PROJECT_CATEGORY_ID, PROJECT_CATEGORY_NAME, P.CATEGORY_SUB_ID AS CATEGORY_SUB_ID, CATEGORY_SUB_NAME,
       CS.CATEGORY_ID AS CATEGORY_ID, CATEGORY_NAME, START_DATE, END_DATE, P.MEMBER_ID AS MEMBER_ID, MEMBER_NAME, NVL(TARGET_AMOUNT,0), ROUND(END_DATE-SYSDATE) AS DDAY,
       TO_CHAR(NVL((SELECT SUM(TOTAL_AMOUNT) FROM PAYMENT PA WHERE PA.PROJECT_ID = P.PROJECT_ID GROUP BY PROJECT_ID),0),'999,999,999,999,999') AS TOTAL_AMOUNT, CONCAT(ROUND(NVL((SELECT SUM(TOTAL_AMOUNT) FROM PAYMENT PA WHERE PA.PROJECT_ID = P.PROJECT_ID GROUP BY PROJECT_ID),0)/NVL(TARGET_AMOUNT,0)*100),'%') AS PERCENT,p.report_count
=======
----------------------------ÆÝµùÇÁ·ÎÁ§Æ® ½æ³×ÀÏ ºä-------------------
