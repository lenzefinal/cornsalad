    ...
    SELECT P.PROJECT_ID, 
           ...
           TARGET_AMOUNT,
            ROUND(NVL((SELECT SUM(NVL(PAYTOTAL.COUNT, 0)) AS SUM_PAY_COUNT
                        ...
                        GROUP BY (PROD.PROJECT_ID)),0)/
                           (CASE
                            WHEN TARGET_AMOUNT > 0 THEN NVL(TARGET_AMOUNT,0)*100
                            WHEN TARGET_AMOUNT = 0 THEN 1 END)) AS PERCENT
    FROM PROJECT P
...
PROJECT_ID, PRODUCT_ID, PRODUCT_NAME, MINCOUNT, REST_COUNT, PAY_COUNT, PAYMENT_DATE_SEVEN
)
AS
    SELECT PROJECT_ID,
       PRODUCT_ID, 
       PRODUCT_NAME,
       MINCOUNT,
       MINCOUNT - PAY_COUNT AS REST_COUNT,
       PAY_COUNT,
       PAYMENT_DATE_SEVEN
FROM(  SELECT PROD.PROJECT_ID,
               PROD.PRODUCT_ID,
               PROD.PRODUCT_NAME,
               PROD.MINCOUNT,
               --PROD.MINCOUNT - NVL(PAYC.COUNT, 0) AS REST_COUNT,
               SUM(NVL(PAYC.COUNT, 0)) AS PAY_COUNT,
               PAYMENT_DATE + 7 AS PAYMENT_DATE_SEVEN
        FROM PRODUCT PROD
        LEFT JOIN PAYMENT_COUNT PAYC ON(PROD.PRODUCT_ID = PAYC.PRODUCT_ID)
        JOIN PROJECT P ON(P.PROJECT_ID = PROD.PROJECT_ID)
        GROUP BY (PROD.PRODUCT_ID,PROD.PROJECT_ID,
               PROD.PRODUCT_NAME,
               PROD.MINCOUNT,PAYMENT_DATE + 7)
