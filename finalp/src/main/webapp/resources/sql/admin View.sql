...
(SELECT COUNT(PROJECT_ID) FROM PROJECT WHERE PROJECT_ONOFF_FLAG='N') AS OFFPROJECTCOUNT,
...
RNUM,REPORT_ID, REPORT_CATEGORY_NAME, PROJECT_ID, PROJECT_REPLY_ID, BOARD_ID, BOARD_REPLY_ID, 
BLACK_NAME, REPORT_REASON, MEMBER_NAME, REPORT_DATE, REPORT_PROJECT_NAME, REPLY_CONTENT, REPORT_COUNT , REPORT_READ_FLAG )
AS SELECT ROWNUM RNUM,REPORT_ID, REPORT_CATEGORY_NAME, PROJECT_ID, PROJECT_REPLY_ID, BOARD_ID, BOARD_REPLY_ID, 
    BLACK_NAME, REPORT_REASON, MEMBER_NAME, REPORT_DATE, REPORT_PROJECT_NAME, REPLY_CONTENT, REPORT_COUNT , REPORT_READ_FLAG
FROM (SELECT R.REPORT_ID, C.REPORT_CATEGORY_NAME, R.PROJECT_ID, R.PROJECT_REPLY_ID, R.BOARD_ID, R.BOARD_REPLY_ID,
...
PROJECT_ONOFF_FLAG,TARGET_AMOUNT,REPORT_COUNT, START_DATE, END_DATE, SPON , ING_FLAG)
AS SELECT ROWNUM RNUM,C.PROJECT_CATEGORY_NAME, CS.CATEGORY_SUB_NAME, P.PROJECT_ID, P.PROJECT_NAME, M.MEMBER_NAME, P.CREATION_DATE ,P.PROJECT_REQUEST_FLAG, 
    P.PROJECT_ONOFF_FLAG,P.TARGET_AMOUNT, P.REPORT_COUNT, P.START_DATE, P.END_DATE, (ROUND(((SELECT SUM(TOTAL_AMOUNT) FROM PAYMENT M 
    WHERE P.PROJECT_ID = M.PROJECT_ID GROUP BY PROJECT_ID)/P.TARGET_AMOUNT)*100)) AS SPON,
    CASE WHEN P.END_DATE >= SYSDATE THEN 'Y' WHEN P.END_DATE < SYSDATE THEN 'N' END AS ING_FLAG
    FROM PROJECT P
...
CREATE OR REPLACE VIEW A_SPONSTAT(
    RANUM, SPON, MEMBER_ID, MEMBER_NAME)
AS SELECT RANUM, SPON, MEMBER_ID, MEMBER_NAME
  FROM (SELECT RANK() OVER(ORDER BY SUM(TOTAL_AMOUNT) DESC) AS RANUM, SUM(TOTAL_AMOUNT) 
          AS SPON, A.MEMBER_ID, M.MEMBER_NAME
        FROM PAYMENT A
