		...
		<form name="purchaseForm" id="purchaseForm" method="post">
                ...
                <ul>
                  ...
                  <c:forEach items="${ idlist }" var="idlist" varStatus="status">
	                  <c:forEach items="${ glist }" var="glist">
	                  	<c:if test="${ (glist.gift_id eq idlist) and (glist.project_id eq p.project_id)}">
            ...
            <div class="wpurchase-delivery-wrap">
              ...
              <div class="wpurchase-notice">
                <p class="title">콘샐러드 펀딩 배송 안내</p>
                <p class="text">펀딩에 대한 보답으로 메이커는 리워드를 약속한 배송일에 제공하기 위해 노력을 다합니다.<br/><br/>
               펀딩의 배송은 메이커가 담당하는 의무로 프로젝트 스토리에 안내한 배송시작일에 배송 시작을 원칙으로 합니다.<br/><br/>
                다만, 프로젝트 진행 중 예기치 못한 사정으로 펀딩 제작 및 배송 일정 변경이 있을 수 있음을 알려드립니다. 변경되는 배송일은 새소식으로 메이커가 직접 전해드립니다.<br/><br/>
                펀딩 제품의 불량 또는 배송오류에 대한 문의 접수 및 교환 처리는 리워드를 제공하는 메이커가 직접 담당하고 있습니다. 메이커에게 문의하기에 기재된 연락처로 직접 소통할 수 있습니다.</p>
              </div>
			...
			<hr>
				<div class="btn-wrap">
    				...
    				<button class="btn-rd-violet" onclick="openPay('${ p.project_name }','${ loginUser.member_id }','${ p.project_id }','${ g_ids }','${ g_amounts }','${ total_account }')">다음 단계로 &gt;</button>
    			</div>
